...
<figure><img src="../../.gitbook/assets/PaymentProvider_2026.png" alt=""><figcaption></figcaption></figure>
...
<strong>    "buyerDappPortalAddress": "{user_wallet_address}",
    ...
    "currencyCode": "{currency_code}",//only optional when pgType is LINE_IAP
    "price": "{price}",//only optional when pgType is LINE_IAP
    "lineIapProductId": "{productId}",
...
<table data-full-width="false"><thead><tr><th width="260">field</th><th>limitation</th></tr></thead><tbody><tr><td>buyerDappPortalAddress</td><td>Max length : 42</td></tr><tr><td>pgType</td><td><ul><li>STRIPE</li><li>CRYPTO</li><li>LINE_IAP</li></ul></td></tr><tr><td>currencyCode</td><td><ul><li>USD</li><li>KRW</li><li>JPY</li><li>TWD</li><li>THB</li><li>KAIA</li><li>USDT</li></ul><p>It should be equal to Item's currencyCode<br><br><mark style="color:blue;">It is optional where pgType is LINE_IAP</mark></p></td></tr><tr><td>price</td><td><ul><li><p>STRIPE</p><ul><li><p>Put minimum unit following <a href="https://en.wikipedia.org/wiki/ISO_4217">here</a></p><ul><li>100(cent) should be put if price is $1 </li></ul></li><li><p>Case1. Minumum unit equals to price unit</p><ul><li>10,000 KRW = 10000</li><li>10,000 JPY = 10000</li></ul></li><li><p>Case2. Minimum unit no equals to price unit</p><ul><li>10,000 USD = 1000000 (10,000 * 100)</li><li>10,000 THB = 1000000 (10,000 * 100)</li><li>10,000 TWD = 1000000 (10,000 * 100)</li></ul></li></ul></li><li><p>CRYPTO</p><ul><li><p>KAIA</p><ul><li>Put price as KAIA unit</li><li>1 KAIA = 1.0</li><li>Up to 4 demcimal places</li></ul></li><li><p>USDT</p><ul><li>1 USDT = 1.0</li><li>Up to 2 decimal places</li></ul></li></ul></li><li><p>LINE_IAP</p><ul><li>No needed</li></ul></li></ul><p></p><p>It should be equal to sum of each price of Items.<br><br>- Decimal Policy<br>STRIPE : n<br><br><mark style="color:blue;">It is optional where pgType is LINE_IAP</mark></p></td></tr><tr><td>lineIapProductId</td><td>You can set price only with LINE IAP's productId.<br>Please refer to productIds for <a href="https://workers-hub.ent.box.com/s/edyvsqzxihhssev4pdus2ixrvayei8wu">LINE IAP ></a><br><br><mark style="color:$danger;">Only Mini App channels approved by LY are authorized to use LINE IAP. If you have any inquiries, please contact the Unifi team.</mark></td></tr><tr><td>paymentStatusChangeCallbackUrl<br><a href="./#id-3.-payment-status-change-event">for more detail</a></td><td><p>Max length : 512<br><br>You can receive webhook whenever payment status was changed if you set paymentStatusChangerCallbackUrl. </p><p><mark style="color:$danger;">Please use port 443 for entire connection. Webhooks cannot be received if a port other than 443 is used.</mark></p></td></tr><tr><td>lockUrl<br><a href="./#id-1.-lock-event">for more detail</a></td><td>Max length : 512</td></tr><tr><td>unlockUrl<br><a href="./#id-2.-unlock-event">for more detail</a></td><td>Max length : 512</td></tr><tr><td>items(*)</td><td>Only the purchase of single item is supported under current version</td></tr><tr><td>testMode</td><td><p>The payment methods according to testMode are as follows.<br><br>- testMode : false<br>STRIPE : real mode<br>CRYPTO : kaia</p><p>LINE_IAP: real mode<br>- testMode : true<br>STRIPE : test mode<br>CRYPTO : kairos<br>LINE_IAP: No supported</p></td></tr></tbody></table>

Items(\*)

<table><thead><tr><th width="182">field</th><th>limitation</th></tr></thead><tbody><tr><td>itemIdentifier</td><td>Max length : 256</td></tr><tr><td>name</td><td>Max length : 256</td></tr><tr><td>imageUrl</td><td>Max length : 512</td></tr><tr><td>price</td><td><p>Put minimum unit following <a href="https://en.wikipedia.org/wiki/ISO_4217">here</a></p><p>For example, put 100(cent) if price is 1 Dollar.<br><br><mark style="color:blue;">It is optional where pgType is LINE_IAP</mark></p></td></tr><tr><td>currencyCode</td><td><ul><li>USD</li><li>KRW</li><li>JPY</li><li>TWD</li><li>THB</li><li>KAIA</li><li>USDT</li></ul><p><br><mark style="color:blue;">It is optional where pgType is LINE_IAP</mark></p></td></tr></tbody></table>
...
<table data-full-width="false"><thead><tr><th width="146.04241943359375">HTTP Status Code</th><th>Description</th></tr></thead><tbody><tr><td>200</td><td><p>Example Value</p><pre class="language-javascript"><code class="lang-javascript">{
 ...
    "buyerDappPortalAddress": "{user_wallet_address}",
    "pgType": "{pg_type}",
    "status": "{status}",
    "currencyCode": "{currency_code}",//If the payment type is LINE IAP and the status is CREATED or CANCELED, currencyCode may be returned as null.
    "price": "{price}",//If the payment type is LINE IAP and the status is CREATED or CANCELED, price may be returned as null.
    "lineIapProductId": "{productId}",
    "usdExchangeRate": {usdExchangeRate}, //Fx rate at the completion of payment. It is only returned where pgType is STRIPE and status is CONFIRMED or FINALIZED.
    "usdExchangePrice": {usdExchangePrice}, //USD Price applied fx rate at the completion of payment. It is only returned where pgType is STRIPE and status is CONFIRMED or FINALIZED.
    "items": [
        {
            "itemIdentifier": "{your_item_identifier}",
            "name": "{your_item_name}",
...
</code></pre></td></tr><tr><td>-31003</td><td><p></p><ul><li>If the IAP permission has not been granted</li><li>If the browser is not a LIFF browser</li><li>If the user is not located in Japan</li><li>If the user has not agreed to the IAP terms and conditions</li></ul><pre class="language-json"><code class="lang-json">{
code: -31003
message: Can not process LINE_IAP payment
}
